...
// One partial sum to force one atomic per WGP instead of 256.
...
void main() {
    ...
    if (n_rows >= 2u) {
        for (uint k = 0u; k < n_repeats; ++k) {
            // Fuck compiler optimizations, imma start throwing bullshit 
            float bias = float(k) * 1e-9;
            float ref = data[col] + bias;
